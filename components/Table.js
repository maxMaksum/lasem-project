...
import{fetchAdd, fetchUpdate, fetchDelete, fetchSearch }from "../components/CrudFunction"
...
const { users, setUsers, showAddForm,setShowAddForm, setSearching, setShowUpdate,showUpdate, setShowSpinner, setUserEdit } = useContext(Store);
...
<table className="table-auto w-2xl mx-auto text-gray-900">
  <thead>
    <tr>
      ...
      <th className="p-2 sm:p-4">Desa</th>
      <th className=" hidden sm:inline-flex p-2 sm:p-4 ">Rt</th>
      <th className="hidden sm:inline-flex p-2 sm:p-4">Rw</th>
      <th className="hidden sm:inline-flex p-2 ">Update</th>
    </tr>
  </thead>
  ...
  (
    ...
    <tr key={person._id}>
        <td  className='p-2 sm:p-4'>{person.rm}</td>
         <td  className='p-2 sm:p-4'>{person.nama}</td>
         <td  className='p-2 sm:p-4'>{person.namakk}</td>
         <td  className=' p-2 sm:p-4'>{person.alamat}</td>
         <td  className='hidden sm:inline-flex w-12 px-4 py-4 '>{person.rt}</td>
         <td  className='hidden sm:inline-flex w-12 px-4  py-4 '>{person.rw}</td>
         <td className='px-4 ' > 
         <div onClick={(e)=>deleteData(e, person._id)} className=' text-green cursor-pointer w-5 h-5 rounded-full  flex items-center justify-center'><CloseIcon /></div>
         <div onClick={(e)=>editData(e, person)} className=' text-green cursor-pointer w-5 h-5 rounded-full  flex items-center justify-center'> <TouchAppIcon/> </div>
        ...
        {/* <div className =" text-sm text-left text-gray-900 dark:text-gray-400 mt-2 ">
            ...
            "text-sm sm:text-md grid grid-cols-12 gap-4 place-content-center my-2 h-10 mx-4 text-gray-900 p-2">
                    <p className="hidden bg-green-600 h-8 p-2">RM</p>
            ...
            <div className='grid grid-cols-12 gap-2'>                   
                ...
                <p className="hidden md:grid col-span-2  rounded p-1 bg-transparent">
                    <div className='flex justify-around items-center'>
                        ...
                        <div  onClick={(e)=>editData(e, customer)} className=' cursor-pointer w-5 h-5 rounded-full bg-transparent flex items-center justify-center'><Border p-2ColorIcon /></div>
